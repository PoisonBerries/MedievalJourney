...
const Armors = {
  ...
  arthurHelmet: { slot: 'helmet', id: 'arthurHelmet', name: "Arthur's Helmet", rangedReduction: 3, icon: 'i_helmet' },
  arthurChest: { slot: 'chest', id: 'arthurChest', name: "Arthur's Chestplate", meleeReduction: 3, icon: 'i_chest' },
};
...
const ItemDefs = {
  ...
  broom: { id: 'broom', name: 'Witch\'s Broom', icon: 'i_potion_blue', desc: 'Instantly sweep away any 1-HP enemy', combatUse: (s, ctx) => { const t = ctx.foes.find(f => f.hp > 0 && f.hp <= 1); if (t) { t.hp = 0; ctx.pushLog(`You sweep ${t.name} clean away!`); } else ctx.pushLog('No weak enough foe to sweep away.'); } },
  marriedIguana: { id: 'marriedIguana', name: 'Married Iguana Potion', icon: 'i_potion_green', desc: '+3 life when drunk', combatUse: (s, ctx) => { GameState.addLife(s, 3); ctx.pushLog('+3 life!'); } },
  coldCane: { id: 'coldCane', name: 'Cold Cane Potion', icon: 'i_potion_blue', desc: 'All weapons +1 damage', combatUse: null },
  mooseJuice: { id: 'mooseJuice', name: 'Moose Juice', icon: 'i_potion_purple', desc: 'Woodland animals leave you be' },
  ...
  pineappleJuice: { id: 'pineappleJuice', name: 'Pineapple Juice', icon: 'i_potion_green', desc: 'Free & yummy', combatUse: (s, ctx) => { ctx.pushLog('Refreshingly free and yummy.'); } },
